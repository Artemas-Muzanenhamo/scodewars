...
class StringCleanSpec extends org.scalatest.flatspec.AnyFlatSpec {
  "stringClean" should "pass basic tests" in {
    val testCases = Seq(
      ("", ""),
      ("! !", "! !"),
      ("123456789", ""),
    ...
    testCases foreach {
      (s, expected) =>
        assert(stringClean(s) == expected, s"for stringClean(\"$s\")")
    }
  }
}